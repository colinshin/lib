...
func GetEngine(name string, ctx context.Context) (*ElasticClient, error) {
	if EsEngine == nil {
		...
		if conf.ElasticNacos.Name != "" {
			...
			ns, e := nacos.GetEngine(conf.ElasticNacos.Name, ctx)
			if e == nil {
	...
	if okC {
		objelastic := newClient(o)
		EsEngine.EsContainer.Set(name, objelastic)
		return objelastic, nil
	}
	logger.AddError(zap.Error(errors.New("no find elastic config " + name)))
	return nil, errors.New("no find elastic config " + name)
}
func newClient(o config.MidEsConf) *ElasticClient {
	c := &sync.Pool{
		New: func() any {
			q, _ := newEngine(o)
			return q
