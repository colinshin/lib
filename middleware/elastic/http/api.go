...
func NewHttpClient(MidEsConf config.MidEsConf) *HttpClient {
	c := new(HttpClient)
...
func (c *HttpClient) initHttp() {
	if c.http == nil {
		if c.gateway.ReadTimeout == "" {
			...
			logger.AddWarn(zap.String("elastic", " read timeout error"+c.gateway.ReadTimeout))
		}
		if c.gateway.WriteTimeout == "" {
			...
			logger.AddWarn(zap.String("elastic", " write timeout error"+c.gateway.WriteTimeout))
		}
		if c.gateway.MaxIdleConn == "" {
			...
			logger.AddWarn(zap.String("elastic", " max idle timeout error"+c.gateway.MaxIdleConn))
		}
...
func (c *HttpClient) activeF() {
	if len(c.cluster.Host) == 0 {
		logger.AddError(zap.String("elastic", c.clusterName+" cluster no address"))
		return
	}
...
func (c *HttpClient) populateNodes(tryNums uint) {
	...
	r, e := c.SendRequest("GET", "/_nodes/_all/http", "", c.readTimeout, 1000, false)
	if e != nil {
		if tryNums == 0 {
			c.populateNodes(tryNums + 1)
			return
		}
		if tryNums == 1 {
			c.cluster = c.gateway
...
func (c *HttpClient) ContextF(ctx context.Context) {
	c.context = ctx
}
...
func (c *HttpClient) ctx(ctx context.Context) {
	c.context = ctx
}

func (c *HttpClient) SendRequest(method string, url string, sendData string, timeout time.Duration, tryNums uint, isRetryConn bool) ([]byte, error) {
	if c.http == nil {
		c.initHttp()
	}
	if c.active == "" {
	...
	if err == nil {
		if statusCode == http.StatusOK {
			...
			logger.AddWarn(zap.Any("elastic", lm))
			if c.FailFun != nil {
		...
		logger.AddWarn(zap.String("elastic", errMsg))
		if isRetryConn {
			c.reset()
		}
		// || errName == "timeout"
		if tryNums < 1 && (errName == "conn_close") {
			return c.SendRequest(method, url, sendData, timeout, tryNums+1, isRetryConn)
		}
		lm := LastMessage{
			RequestMethod: string(req.Header.Method()),
			RequestUrl:    string(req.Header.Host()) + string(req.Header.RequestURI()),
