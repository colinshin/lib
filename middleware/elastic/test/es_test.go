...
func TestBody(t *testing.T) {
	...
	ss = e.GetSearchService(context.Background())
	//body 可以写原生的body
...
func TestWhere(t *testing.T) {
	...
	w1 := ss.GetNewWhere()
	_ = w1.WhereIn("id", ss.FieldIntArray([]int{5, 6, 7}))
	w2 := ss.GetNewWhere()
	_ = w2.WhereIn("id", ss.FieldIntArray([]int{8, 9, 10}))
...
func TestMatch(t *testing.T) {
	...
	_ = tq1.Where(ss.FieldMatchPrefix("name", "管理"))
	tq2 := ss.GetNewWhereMatch()
	_ = tq1.Where(ss.FieldMatch("name", "管理"))
	m = append(m, tq1, tq2)
	ss.OrWhereMatchM(m)
