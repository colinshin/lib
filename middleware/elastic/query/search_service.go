...
// 获取一个serche服务
func GetSearchService(engine QEngine, ctx context.Context) *SearchSevice {
	s := new(SearchSevice)
	s.InitService(engine)
	s.context = ctx
	s.httpClient.ContextF(ctx)
	s.where.ContextF(ctx)
