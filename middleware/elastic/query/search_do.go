...
func (sd *searchDo) DeleteScroll(s *SearchSevice, scrollId string) error {
	...
	_, e := s.httpClient.SendRequest(fasthttp.MethodDelete, "/_search/scroll", body, s.timeOut, 0, true)
	return e
}
func (sd *searchDo) BatchByScroll(s *SearchSevice, scrollId string) (error, *ResultScroll) {
	start := time.Now()
	r, e := s.httpClient.SendRequest(fasthttp.MethodGet, sd.getScrollUrl(s, scrollId), "", s.timeOut, 0, true)
...
func (sd *searchDo) Do(s *SearchSevice) ([]byte, error) {
	...
	s.Dsl = body
	start := time.Now()
	r, e := s.httpClient.SendRequest(fasthttp.MethodPost, sd.getUrl(s), body, s.timeOut, 0, true)
	tTime := time.Since(start).Milliseconds()
	logger.AddEsTime(int(tTime))
	return r, e
}
